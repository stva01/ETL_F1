...
export default async function ComparePage() {
  ...
  const [drivers, trendData] = await Promise.all([
    querySnowflake<DriverStat>(`
      SELECT 
      ...
      LIMIT 100
    `),
    querySnowflake<{ driver_key: number, season_year: number, season_wins: number }>(`
      ...
      WHERE driver_key IN (
        SELECT driver_key FROM MARTS.mart_driver_career ORDER BY total_race_starts DESC LIMIT 100
      )
    `)
  ]);
